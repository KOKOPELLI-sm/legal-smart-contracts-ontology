...
def contract_to_triples(slith):
    ...
    # Classes
    for cls in ["SmartContract", "Function", "StateVariable", "Struct", "Member", "Parameter", "Event", "Modifier"]:
    ...
    # --- Helper functions for instance triples ---
    def add_property_with_domain_range(prop, domain_class, range_class):
        # Already declared above, so do nothing here
        pass
    def define_data_property(prop, domain, dtype=XSD.string):
        # Already declared above, so do nothing here
        pass
    triples = Graph()
    triples.bind("ex", EX)
    ...
    for contract in slith.contracts:
        ...
        for var in contract.state_variables:
            ...
            if any(k in var.name.lower() for k in ('amount', 'deposit', 'price', 'rent')):
